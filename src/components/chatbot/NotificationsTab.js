...
import   './NotificationsTab.module.css'
...
export class Notifications extends Component {
  ...
  createTextMessageNitification(from, messageText){ return (<><h4 onClick={()=>this._onNotificationTabClick(from)} className='notifiitem_heading'><b>{from}</b> Send a new message</h4>
            <p className='notifiitem_paragraph'>{messageText}</p></>)}


  createFileMessageNitification(from, props){     
    return (<><h4 className='notifiitem_heading'><b>{from}</b> Sent you an {props.message.msg.files? "File":"Image"}</h4>
    {/* <p className={noti_styles.notifiitem_paragraph}>{from}</p> */}
    {props.message.msg.images && <img src={props.message.msg.images[0]}></img>}
    {props.message.msg.files && <p className='notifiitem_paragraph'>You have an file Attachement</p>}
    </>)}
  ...
  render() {
      return ( 
        <div className='notifiitem' onClick={()=>this.props.fetchChatHistory(this.props.conversation_id)}>
          <img className='notifiitem_img' src={get_Picture(this.props.message.from)} alt="img" />
