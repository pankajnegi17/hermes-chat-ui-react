  ...
  render() {
    console.warn(this.props.fresh_notifications)
      ...
      let notifications_items = this.props.all_notifications.map(notification=>{
          return ( <div className='notifiitem' onClick={()=>this.props.onNotificationItemClicked(notification)}>
          <img className='notifiitem_img' src={get_Picture(notification.from)} alt="img" />
          <div className='noti_content'>
          <h4 className='notifiitem_heading'><b>{notification.fname}</b> Send <b> {notification.count}</b> new messages</h4>
